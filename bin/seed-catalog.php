...
/**
 * Semilla inicial: certificadores, proveedores y productos estrella / comunes.
 * Uso: php bin/seed-catalog.php
 */
...
$upsertProduct = static function (array $row) use ($pdo): void {
...
foreach ($products as $p) {
    ...
    $upsertProduct($p);
    fwrite(STDOUT, "Producto: {$p['code']}\n");
}

// Combo ejemplo TOEFL + CENNI + prep = 2500
...
fwrite(STDOUT, "Combo TOEFL-FULL listo.\n");
...
foreach ($pipelines as [$code, $name, $type, $steps]) {
    ...
    foreach ($steps as $i => [$scode, $label, $actor]) {
        $ins->execute([$tid, $scode, $label, $actor, $i, $scode === 'fin' ? 1 : 0]);
    }
    fwrite(STDOUT, "Pipeline: {$code}\n");
}

fwrite(STDOUT, "Seed completado.\n");
exit(0);
