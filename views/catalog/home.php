...
<div class="layout-catalog">
    ...
    <section>
        <div class="toolbar">
            <form class="search" method="get" action="<?= e(url('/catalogo')) ?>" id="catalog-search-form">
                ...
                <input type="search" name="q" id="catalog-search-input" value="<?= e($q) ?>"
                       placeholder="Escribe para filtrar (nombre, código, certificadora…)"
                       autocomplete="off">
        ...
        <?php if ($products === []): ?>
            <div class="empty" id="catalog-empty-server">Aún no hay productos públicos. El admin puede cargarlos en <strong>Admin → Productos</strong>.</div>
        <?php else: ?>
            <?php
            $remaining = 0;
            $hasMorePages = false;
